...
# DEFAULT_FAN_VALUE = 19
# DEFAULT_FAN_VALUE = 9
DEFAULT_FAN_VALUE = 7+3
...
# Omega to consider it a spectral peak. Higher values mean less
# fingerprints and faster matching, but can potentially affect accuracy.
# PEAK_NEIGHBORHOOD_SIZE = 20
# PEAK_NEIGHBORHOOD_SIZE = 25
PEAK_NEIGHBORHOOD_SIZE = 15+15
...
FINGERPRINT_REDUCTION = 20+10

IDX_FREQ_I = 0
IDX_TIME_J = 1
# net+=1

def fingerprint(count, channel_samples, Fs=DEFAULT_FS,
    ...
    path = 'plots/signal/'
    plt.savefig(path + '{}.png'.format(count), dpi=fig.dpi)
    plt.clf()
    plt.close()
    ...
    arr2D = 10 * np.log10(arr2D, out=np.zeros_like(arr2D),
                          where=(arr2D != 0))  # calculates the base 10 logarithm for all elements of arr2D
    arr2D[arr2D == -np.inf] = 0  # replace infs with zeros

    # find local maxima
    local_maxima = list(get_2D_peaks(arr2D, count, amp_min=amp_min))
...
def get_2D_peaks(arr2D, count, amp_min=DEFAULT_AMP_MIN, ):
